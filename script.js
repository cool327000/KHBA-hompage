...
const springStandings=[['DOOSAN BEARS','—','—','—','—','—','—','—','—','—','—','—','—','—','—','—'],['LG TWINS','—','—','—','—','—','—','—','—','—','—','—','—','—','—','—'],['HANSHIN TIGERS','—','—','—','—','—','—','—','—','—','—','—','—','—','—','—'],['CANBERRA CAVALRY','—','—','—','—','—','—','—','—','—','—','—','—','—','—','—']];
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 const standingTabs=document.querySelectorAll('[data-standing-tab]');
 function renderStandings(data){
  if(standingsBody) standingsBody.innerHTML=data.map(t=>`<tr><td><strong>${t[0]}</strong></td><td>${t[1]}</td><td>${t[2]}</td><td>${t[3]}</td><td>${t[4]}</td><td>${t[5]}</td><td>${t[6]}</td><td>${t[7]}</td><td>${t[8]}</td><td>${t[9]}</td><td>${t[10]}</td><td>${t[11]}</td><td>${t[12]}</td><td>${t[13]}</td><td>${t[14]}</td><td>${t[15]}</td></tr>`).join('');
 }
 if(standingsBody){
  renderStandings(regularStandings);
  standingTabs.forEach(tab=>tab.addEventListener('click',()=>{
   standingTabs.forEach(t=>t.classList.remove('active'));
   tab.classList.add('active');
   renderStandings(tab.dataset.standingTab==='spring'?springStandings:regularStandings);
  }));
 }
